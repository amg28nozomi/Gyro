...
namespace Gyro {
  namespace Player {
    ...
    bool Player::Init() {
      ...
      // アタックコンポーネントの設定
      // 当たり判定は球をセットする
      _attack = std::make_unique<Object::AttackComponent>(*this, std::move(std::make_shared<Object::CollisionSphere>(*this, _position, 10.0f)));
    ...
    void Player::Attack() {
      ...
      // アニメーションからローカル座標を取得
      auto pos = MV1GetAttachAnimFrameLocalPosition(_model, animIndex, 0);
      // ローカル座標を攻撃座標にセットする
